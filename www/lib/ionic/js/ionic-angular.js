 ...
 * of your app. Additionally, `$ionicConfig` can also set and get views values during the run
 * phase and within the app itself.
 *
 * By default, all base views variables are set to `'platform'`, which means it'll take on the
 * default views of the platform on which it's running. Config variables can be set at this
 * level so all platforms follow the same setting, rather than its platform views.
 * The following code would set the same views variable for all platforms:
 *
 * ```js
 * $ionicConfigProvider.views.maxCache(10);
 * ```
 *
 * Additionally, each platform can have it's own views within the `$ionicConfigProvider.platform`
 * property. The views below would only apply to Android devices.
 *
 * ```js
 ...
 *
 * myApp.views(function($ionicConfigProvider) {
 ...
 * instance. Basically, any forward views are reset each time. Set this views to `true` to have
...
.provider('$ionicConfig', function() {
  ...
  // private: create methods for each views to get/set
  function createConfig(configObj, providerObj, platformPath) {
    forEach(configObj, function(value, namespace) {

      if (angular.isObject(configObj[namespace])) {
        // recursively drill down the views object so we can create a method for each one
        providerObj[namespace] = {};
        createConfig(configObj[namespace], providerObj[namespace], platformPath + '.' + namespace);

      } else {
        // create a method for the provider/views methods that will be exposed
        providerObj[namespace] = function(newValue) {
          ...
          if (configObj[namespace] == PLATFORM) {
            // if the views is set to 'platform', then get this views's platform value
            var platformConfig = stringObj(configProperties.platform, ionic.Platform.platform() + platformPath + '.' + namespace);
            if (platformConfig || platformConfig === false) {
              return platformConfig;
            }
            // didnt find a specific platform views, now try the default
 ...
 *   .views(function($stateProvider, $ionicConfigProvider) {
 *
 ...
 *         templateUrl: "tabs-templates/menu.html"
 *       })
 *       .state('tabs.home', {
 *         url: "/home",
 *         views: {
...
function($stateProvider, $ionicConfigProvider) {
  ...
  $stateProvider.state = function(stateName, definition) {
    // don't even bother if it's disabled. note, another views may run after this, so it's not a catch-all
...
function($timeout, $document, $q, $ionicClickBlock, $ionicConfig, $ionicNavBarDelegate) {
      ...
      emit: function(step, enteringData, leavingData) {
        ...
        cleanup: function(transData) {
          ...
          if (leavingEle && transData.direction == 'back' && !$ionicConfig.views.forwardCache()) {
            ...
            // do not remove forward views if cacheForwardViews views is true
  ...
  function getTransitionData(viewLocals, enteringEle, direction, view) {
    ...
    // 3) entering view's $state views property
 ...
 *   `<preference name="Fullscreen" value="false" />` or no preference in your `views.xml` file,
 ...
 * ```js
 * var app = angular.module('myApp', ['ionic']);
 * app.views(function($stateProvider) {
 *   $stateProvider
 *   .state('index', {
 *     url: '/',
